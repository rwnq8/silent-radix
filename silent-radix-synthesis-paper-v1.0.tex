...
% arXiv submission: math.HO (History and Overview)
...
% --- Essential packages (arXiv-compatible) ---
...
\hypersetup{
 ...
 pdftitle={The Silent Radix: Positional Notation as Ultrametric Tree and the Calculus of Indications as Remedy},
 pdfsubject={math.HO},
 pdfkeywords={positional notation, ultrametric, silent radix, Laws of Form, p-adic}
}
...
% --- Title ---
\title{The Silent Radix:\\Positional Notation as Ultrametric Tree\\and the Calculus of Indications as Remedy}
...
the ultrametric already native to positional notation. Re-founding
quantitative representation on Spencer-Brown's calculus of indications
--- where drawing a distinction is marking a cycle --- restores the base
as an explicit phenomenal frame, the ultrametric as the intrinsic
geometry of nested distinctions, zero as the unmarked root, and the
self-referential ``10'' as a stable re-entrant form: the cycle measuring
its own completion, which is the minimal observer and the formal origin
of self-aware measurement.
\end{abstract}

\tableofcontents
\newpage

\section{1. Introduction: The ``10'' Misnomer as Primal
...
\section{2. The Silent Radix in Practice: A Catalog of
Consequences}\label{the-silent-radix-in-practice-a-catalog-of-consequences}
...
\subsection{2.1 Computing and Software
Engineering}\label{computing-and-software-engineering}

The C programming language's leading-zero-octal convention (Ritchie,
1993) is the paradigmatic silent radix error. The literal \texttt{010}
is parsed as octal (decimal 8) rather than decimal 10 because the
leading zero --- originally inherited from BCPL as a pragmatic notation
for machine addresses --- serves as an implicit radix marker. This has
produced hundreds of documented Common Vulnerabilities and Exposures
(CVE) entries, including authentication bypasses where permission masks
were silently reinterpreted.

JSON (RFC 8259) mandates decimal-only number representation. Any number
computed in hexadecimal, octal, or binary loses its radix on
serialization --- information is destroyed without warning. The YAML 1.1
``Norway problem,'' where the country code \texttt{NO} is parsed as
boolean \texttt{false} (type coercion), is structurally identical: a
silent interpretive rule applied to an unannotated token.

Other documented instances include: JavaScript's \texttt{parseInt("08")}
returning \texttt{0} when the radix parameter is omitted; Excel
stripping leading zeros from identifiers treated as numbers; IPv4
address parsing where \texttt{010.0.0.1} becomes \texttt{8.0.0.1}; and
embedded systems where binary sensor data is displayed as decimal due to
missing BCD/radix metadata.

\subsection{2.2 Science and Statistics}\label{science-and-statistics}

The silent metric --- the assumption that the number line is straight
and equally spaced --- produces an analogous class of errors. The most
pervasive is the treatment of Likert-scale survey data (ordinal) as
interval-level data suitable for means, t-tests, and ANOVA. Jamieson
(2004) documented decades of published research using parametric tests
on ordinal data without justification.

This error is structurally identical to the silent radix error: a
parameter (the metric / scale type) that determines the validity of
operations (means vs.~medians, Pearson r vs.~Spearman \(\rho\)) is left
implicit and then defaulted to a culturally convenient but inappropriate
value.

Clinical pain scales rated 0--10 on a Numerical Rating Scale (NRS) are
analyzed parametrically despite pain perception following Steven's power
law with exponent \textasciitilde3.5 --- profoundly nonlinear. The
integers 0--10 import the silent metric of equal spacing onto a domain
where equal spacing is empirically absent.

\subsection{2.3 Physical Units: The Most Catastrophic
...
\subsection{2.4 The Universal Failure
...
\section{3. The Genealogy of the Silent
Frame}\label{the-genealogy-of-the-silent-frame}

\subsection{3.1 Pre-Positional Systems: Transparency Without
...
\subsection{3.2 The Positional Revolution: Power Through
...
\subsection{3.3 The Flattening: From Tree to
...
\subsection{3.4 Digital Recapitulation}\label{digital-recapitulation}
...
\section{4. The Mathematical Demonstration: p-adic Numbers Recover
the Native
Ultrametric}\label{the-mathematical-demonstration-p-adic-numbers-recover-the-native-ultrametric}

\subsection{4.1 Positional Notation Is Already
...
\subsection{4.2 p-adic Numbers as
...
\subsection{4.3 Ostrowski's Theorem and the Contingency of the
...
\section{5. The Epistemological Core: Ontology, Epistemology, and the
Gap}\label{the-epistemological-core-ontology-epistemology-and-the-gap}

\subsection{5.1 The Use/Mention
...
\subsection{5.2 The Collapse and Its
...
\section{6. The Remedy: Laws of Form as a Foundation of Explicit
Distinction}\label{the-remedy-laws-of-form-as-a-foundation-of-explicit-distinction}

\subsection{6.1 Set Theory's Hidden
...
\subsection{6.2 The Calculus of
...
\subsection{6.3 ``10'' as Stable Re-entrant
...
\subsection{6.4 Recovery of the Euclidean
...
\section{7. The Thesis Stated}\label{the-thesis-stated}
...
\section{8. Nine Principles for a Cyclic-Frame Quantitative
...
\section{9. Conclusion: From Misnomer to
Foundation}\label{conclusion-from-misnomer-to-foundation}

The conversation that began with a joke about ``10'' ends with a new
foundation for quantitative representation. The silent radix is not a
quirk to be tolerated but the signature of a structural law:
representation requires interpretation, and interpretation requires an
explicit frame. When the frame is hidden --- in a C literal, a JSON
payload, a Likert scale, or a spacecraft trajectory file --- error
becomes inevitable.

The calculus of indications offers a foundation in which frames cannot
be hidden, because every form is an act of distinction and every
distinction carries its boundary visibly. In this foundation, the ``10''
misnomer is transformed from a bug into the engine of self-aware
measurement --- the tree observing its own branching, the cycle counting
its own completion.
...
\bibitem{ref1} Dehaene, S. (1997). \emph{The Number Sense.} Oxford University Press.
\bibitem{ref2} Descartes, R. (1637). \emph{La Géométrie.}
\bibitem{ref3} Goodman, N. (1968). \emph{Languages of Art.} Hackett.
\bibitem{ref4} Gödel, K. (1931). ``Über formal unentscheidbare Sätze.''
 \emph{Monatshefte für Mathematik und Physik.}
\bibitem{ref5} Hensel, K. (1897). ``Über eine neue Begründung der Theorie der
 algebraischen Zahlen.'' \emph{Jahresbericht der DMV.}
\bibitem{ref6} Hofstadter, D. (1979). \emph{Gödel, Escher, Bach.} Basic Books.
\bibitem{ref7} Jamieson, S. (2004). ``Likert scales: how to (ab)use them.''
 \emph{Medical Education.}
\bibitem{ref8} Kauffman, L. (1987). ``Arithmetic in the Calculus of Indications.''
 \emph{International Journal of General Systems.}
\bibitem{ref9} Maturana, H. \& Varela, F. (1980). \emph{Autopoiesis and Cognition.}
 Reidel.
\bibitem{ref10} Ostrowski, A. (1916). ``Über einige Lösungen der
 Funktionalgleichung.'' \emph{Acta Mathematica.}
\bibitem{ref11} Quine, W.V.O. (1940). \emph{Mathematical Logic.} Harvard University
 Press.
\bibitem{ref12} Ritchie, D. (1993). ``The Development of the C Language.'' \emph{ACM
 HOPL-II.}
\bibitem{ref13} Schikhof, W. (1984). \emph{Ultrametric Calculus.} Cambridge University
 Press.
\bibitem{ref14} Siegler, R. \& Opfer, J. (2003). ``The Development of Numerical
 Estimation.'' \emph{Psychological Science.}
\bibitem{ref15} Smith, B.C. (1996). \emph{On the Origin of Objects.} MIT Press.
\bibitem{ref16} Spencer-Brown, G. (1969). \emph{Laws of Form.} Allen \& Unwin.
\bibitem{ref17} Stevens, S.S. (1946). ``On the Theory of Scales of Measurement.''
 \emph{Science.}
\bibitem{ref18} Stevin, S. (1585). \emph{De Thiende.}
\bibitem{ref19} Tarski, A. (1933). ``The Concept of Truth in Formalized Languages.''
\bibitem{ref20} Varela, F. (1979). \emph{Principles of Biological Autonomy.}
 North-Holland.
\bibitem{ref21} von Foerster, H. (1974). \emph{Cybernetics of Cybernetics.}
\bibitem{ref22} Wittgenstein, L. (1956). \emph{Remarks on the Foundations of
